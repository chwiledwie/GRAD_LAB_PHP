...
?>
    ...
    <tr><td><a href="statPiw.php#kom">Pracownicy posortowani wg. ilości wypożyczeń</a></td></tr>
      ...
      <p class="pasekK">Czytelnicy posortowani wg. ilości wypożyczeń - konkurs:</p>
...
$wynik = mysql_query($utf8) or die(mysql_error());  
    ...
    $zapytanie3 = mysql_query("SELECT *,COUNT(IDWyp) FROM wypozyczenia GROUP BY IDWyp ORDER BY COUNT(IDWyp) DESC;"); //wybiera tabelę i pobiera z niej wszystkie dane


if ($zapytanie3) {  //jeśli zapytanie można wykonać to...
    while ($shit3 = mysql_fetch_array($zapytanie3)) { //robi pętlę i przypisuje wszystkie dane do zmiennej
   
       
    $IDWyp2=$shit3['IDWyp'];
    
    $queryK3 = "SELECT * FROM wypozyczajacy WHERE IDWyp='$IDWyp2';";
    $wynikK3 = mysql_fetch_row( mysql_query( $queryK3 ) );
    
   
    
echo' <tr><td>Wypożyczonych: '.$shit3['COUNT(IDWyp)'].', Nazwisko: '.$wynikK3[3].', Imię: '.$wynikK3[4].'</td>
