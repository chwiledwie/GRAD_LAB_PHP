...
?>
    ...
    <tr><td><a href="statPiw.php#kom">Pracownicy posortowani wg. ilości wypożyczeń</a></td></tr>
      ...
      <p class="pasekK">Książki posortowane wg. ilości wypożyczeń:</p>
...
$wynik = mysql_query($utf8) or die(mysql_error());  
    ...
    $zapytanie2 = mysql_query("SELECT *,COUNT(IDK) FROM wypozyczenia GROUP BY IDK ORDER BY COUNT(IDK) DESC;"); //wybiera tabelę i pobiera z niej wszystkie dane


if ($zapytanie2) {  //jeśli zapytanie można wykonać to...
    while ($shit2 = mysql_fetch_array($zapytanie2)) { //robi pętlę i przypisuje wszystkie dane do zmiennej
   
       
    $IDKK2=$shit2['IDK'];
    
    $queryK2 = "SELECT * FROM ksiazka WHERE IDK='$IDKK2';";
    $wynikK2 = mysql_fetch_row( mysql_query( $queryK2 ) );
    
    $IDAAA2=$wynikK2[1];
     $queryA2 = "SELECT * FROM autor WHERE IDA='$IDAAA2';";
    $wynikA2 = mysql_fetch_row( mysql_query( $queryA2 ) );
    
echo' <tr><td>Wypożyczonych: '.$shit2['COUNT(IDK)'].', Tytuł: '.$wynikK2[5].', Autor: '.$wynikA2[1].' '.$wynikA2[2].', Rok wydania: '.$wynikK2[6].'</td>
