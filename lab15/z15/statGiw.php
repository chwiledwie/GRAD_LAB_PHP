...
?>
    ...
    <tr><td><a href="statPiw.php#kom">Pracownicy posortowani wg. ilości wypożyczeń</a></td></tr>
      ...
      <p class="pasekK">Gatunki posortowane wg. ilości wypożyczonych książek:</p>
   <table class="tabelkaG">
    <?php
...
$wynik = mysql_query($utf8) or die(mysql_error());  
    ...
    $zapytanie5 = mysql_query("SELECT *,COUNT(IDG) FROM wypozyczenia GROUP BY IDG ORDER BY COUNT(IDG) DESC;"); //wybiera tabelę i pobiera z niej wszystkie dane


if ($zapytanie5) {  //jeśli zapytanie można wykonać to...
    while ($shit5 = mysql_fetch_array($zapytanie5)) { //robi pętlę i przypisuje wszystkie dane do zmiennej
   
       
   
    
    $IDG6=$shit5['IDG'];
     $queryK6 = "SELECT * FROM gatunek WHERE IDG='$IDG6';";
    $wynikK6 = mysql_fetch_row( mysql_query( $queryK6 ) );
    
    
   
    
echo' <tr><td>Wypożyczeń: '.$shit5['COUNT(IDG)'].', Gatunek: '.$wynikK6[1].'</td>
